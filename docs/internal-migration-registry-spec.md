...
**남는 약점**: ① **미등기 임차**(확정일자부 임차보증금)은 등기부에 없음 → 전입세대열람 등 별도(범위 밖이면 명시) ② 신선도 ③ 부동산고유번호↔신청 매핑(§7-5).
...
5. ★ **부동산고유번호 ↔ 신청(complex_id/dong/ho) 매핑 경로**: 가장 중요한 미결. 현 `registry_ic_id`(Integer 발급ID)는 폐쇄망에서 무의미. 후보 — ① 신청 시 등기 고유번호 직접 입력 ② 주소(표제부 평문 + 법정동코드 `STDNG_CD`)로 단지/호 역매칭(신뢰도 이슈). → 대화형 결정 필요.
6. `RLES_DVCD` 집합건물 외(단독/토지/상가) 케이스 표제부 레이아웃.
7. **조회 트리거 주체**: NICE 전문을 누가 호출해 적재하나(심사 시스템 자동? 우리가 트리거?). 신청→권리분석 타이밍 설계에 직결.
8. (명세 드리프트) `NICE_MSGM_NO` 길이(샘플 10 vs 명세 14), `GD_USG_NM`/`HDR_CTNT` 데이터타입 NUMBER 오기(실제 텍스트), 저당 `RLES_UNQ_NO` 길이 공란 — DDL 확정 시 정정.

## 8. 다음 단계

step4 사양 확정 → **`registry_db_service` 구현 + `PropertyRightsData` 결정적 빌드 + 더미 차단 + 신선도 UI**. 적재는 step2 ETL(수집기)과 함께. **선결: §7-5 부동산고유번호 매핑 결정** — 이게 정해져야 신청→권리분석 연결 코드 작성 가능.
