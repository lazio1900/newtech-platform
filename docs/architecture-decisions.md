...
## ADR-005. 인증: JWT (Access only, 단기 만료)

**결정**:
- 패스워드: **bcrypt** 해싱
- 토큰: JWT (HS256), Access Token만, **만료 60분**
- 저장: 프론트엔드 **메모리 + httpOnly 쿠키 옵션** (XSS 방지)
- Refresh token은 단계 2에서 검토 (지금은 60분 후 재로그인 허용)

**근거**: 단순성. 심사 도메인은 세션이 짧고 보안 민감. Refresh는 운영 데이터로 필요성 검증 후 도입.
