...
3. mortgage_entries ← 요약 "3. (근)저당권 및 전세권 등 ( 을구 )" 가 "- 기록사항 없음" 이면 빈 array.
   **표가 있으면 그 표의 모든 행을 활성 mortgage_entries 로 빠짐없이 포함하세요. (요약 표 행 수 = mortgage_entries 길이)** 요약에 있는 항목은 모두 현재 유효한 권리이며 절대 말소가 아닙니다.
   특히 다음을 절대 누락·병합하지 말 것:
   - **부기등기**: 순위번호가 "6-1", "2-1" 처럼 'N-M' 형식인 행 — 주 등기와 별개의 행으로 포함.
   - **근질권/근저당권부채권 근질권설정**: "근저당권자" 대신 **"채권자"**(예: 제이비우리캐피탈주식회사)로 표기되지만 별도의 활성 권리이므로 반드시 포함. rank_number 는 "6-1" 그대로, purpose 는 "근질권", target_owner 는 요약 표의 '대상소유자' 열 값.
