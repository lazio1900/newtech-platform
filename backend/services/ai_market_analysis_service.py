...
- "jb_적정시세": JB 적정시세 최종값을 인용한 뒤, prompt 의 "수행달 가중치(KB %·실거래 %)" 와 "산출 근거" 항목(수행달, 그 달 KB/실거래 평균과 표본 수)을 **수치 그대로 들어** 1~2문장으로 풀이. 기본 가중치는 KB 40% / 실거래 60% 이며, 수행달에 실거래 표본이 없으면 KB 단독(100%)으로 폴백. 산출은 그 달의 IQR-제거 평균값들의 가중평균임을 명시.
...
def build_prompt(
    ...
    if jd:
        ...
        jb_section = (
            ...
            f"- 수행달 가중치: KB {w.get('kb', 0)*100:.0f}% / 실거래 {w.get('molit', 0)*100:.0f}%\n"
