...
- "jb_적정시세": JB 적정시세 최종값을 인용한 뒤, prompt 의 "신청월 가중치(KB %·실거래 %)" 와 "산출 근거" 항목(신청월, 그 달 KB/실거래 평균과 표본 수)을 **수치 그대로 들어** 1~2문장으로 풀이. 기본 가중치는 KB 40% / 실거래 60% 이며, 신청월에 실거래 표본이 없으면 KB 단독(100%)으로 폴백. 산출은 그 달의 IQR-제거 평균값들의 가중평균임을 명시.
...
def build_prompt(
    ...
    if jd:
        ...
        jb_section = (
            ...
            f"- 신청월 가중치: KB {w.get('kb', 0)*100:.0f}% / 실거래 {w.get('molit', 0)*100:.0f}%\n"
...
위 사실만 사용해 다음 7개 키를 모두 포함한 JSON 객체로 응답하세요:
{{"kb_시세": "...", "실거래가": "...", "jb_적정시세": "...", "예측": "...", "ltv_검토": "...", "인근_유사물건": "...", "종합_의견": "..."}}
"""
